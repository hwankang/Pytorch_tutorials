...
class LSTM(nn.Module):
    def __init__(self,input_dim,hidden_dim,output_dim,n_layers):
        ...
        self.lstm = nn.LSTM(input_dim, hidden_dim,n_layers,dropout=0.7, batch_first=True)
...
lstm = LSTM(input_dim,hidden_dim,output_dim,n_layers)
print(lstm)
...
for i in range(0,4):
    for idx, step in enumerate(range(steps)):
        ...
        y_Tensor = torch.Tensor(y_data) # label -> (25,1) : 텐서
...
print(len(prediction_target))

plt.plot(target_new,'r',label = 'target')
